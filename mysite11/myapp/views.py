...
def signup(request):#역시 GET/POST 방식을 사용하여 구현한다.
    if request.method == "GET":
        return render(request, 'myapp/signup.html', {'f':SignupForm()} )
...
#즉 사용자가 입력한 password와 password_check가 맞는지 확인하기위해 작성해주었다.
                ...
                client_code=code,
                MetamastAddress=form.cleaned_data['metamask'])
...
#나머지 입력하지 못한 last_name과, first_name은 따로 지정해준다.
                ...
                return render(request, 'myapp/signup.html',{'f':form, 'error':'비밀번호와 비밀번호 확인이 다릅니다.'})#password와 password_check가 다를 것을 대비하여 error를 지정해준다.

        else: #form.is_valid()가 아닐 경우, 즉 유효한 값이 들어오지 않았을 경우는

            return render(request, 'myapp/signup.html',{'f':form})
